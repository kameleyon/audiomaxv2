...
| Commits on `main` | The founding documents, the working agreement, the gate tool and **32 audit records**. The design itself is **not** in the repository — `resources/specs` and `resources/roadmap` are gitignored — and Jury's most recent *recorded* ruling on it is `PASS WITH FIXES` (round 31), which you can read in `resources/audits/`. The commit gate governs the file set under review; the trail is committed so it cannot go missing again. |
| `docs/` (public documentation) | **Present** — created in Phase 0. 6 ADRs in [`docs/architecture/`](docs/architecture/) and the [glossary](docs/glossary.md). `docs/help/` (Guide) does not exist yet. |
...
- **Jury — round 31, `PASS WITH FIXES`:** 0 Blocker · 0 Critical · 9 Major ·
  4 Minor · 0 Polish. A commit is permitted; nine Majors are open, each with a
  named owner and a date.
- **Halo — round 26, `FORECLOSES`:** 2 Blocker · 3 Critical · 8 Major · 4 Minor,
  six items blocking a commit. **Halo has not ruled since**, so this is the
  standing accessibility verdict; it is *older* than Jury's and it is not
  superseded by it. Halo cannot issue `PASS` before implementation and has not.

Rounds 18, 19, 25, 28 and 30 also returned `PASS WITH FIXES`; the `FAIL`s of
rounds 17, 22, 27 and 29 are superseded. Read `resources/audits/` for ground
truth.
...
| 4 | [`docs/architecture/`](docs/architecture/) | 4 | The 6 decisions that shape everything, and **why** — including the one that was reversed, and the one that shows the word-sync bar is ASR-bound |
